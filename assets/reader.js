...
const cacheStartInput = document.getElementById('cache-start');
const cacheEndInput   = document.getElementById('cache-end');
const btnCacheRange   = document.getElementById('btn-cache-range');
const cacheStatusEl   = document.getElementById('cache-status');
const settingsTabReadingEl = document.getElementById('settings-tab-reading');
const settingsTabVocabEl = document.getElementById('settings-tab-vocab');
const settingsPaneReadingEl = document.getElementById('settings-pane-reading');
const settingsPaneVocabEl = document.getElementById('settings-pane-vocab');
const wfStartInput = document.getElementById('wf-start');
const wfEndInput = document.getElementById('wf-end');
const wfMinCountInput = document.getElementById('wf-min-count');
const wfLangJaInput = document.getElementById('wf-lang-ja');
const wfLangEnInput = document.getElementById('wf-lang-en');
const btnWfRecent5 = document.getElementById('btn-wf-recent5');
const btnWfRun = document.getElementById('btn-wf-run');
const wfStatusEl = document.getElementById('wf-status');
const wfResultsEl = document.getElementById('wf-results');
...
function setStudyMode(active) {
    ...
    if (next) {
        flushReadingSession({ reset: false, useBeacon: false });
        isStudyMode = true;
        return;
    }
    ...
    if (readingSession) {
        readingSession.endAt = now;
    }
    lastReadMarkAt = Date.now();
}

function switchSettingsTab(tab) {
    const target = (tab === 'vocab') ? 'vocab' : 'reading';
    if (settingsTabReadingEl) settingsTabReadingEl.classList.toggle('active', target === 'reading');
    if (settingsTabVocabEl) settingsTabVocabEl.classList.toggle('active', target === 'vocab');
    if (settingsPaneReadingEl) settingsPaneReadingEl.classList.toggle('active', target === 'reading');
    if (settingsPaneVocabEl) settingsPaneVocabEl.classList.toggle('active', target === 'vocab');

    setStudyMode(target === 'vocab');
}

function normalizeWordFrequencyInputs() {
    if (!wfStartInput || !wfEndInput || !wfMinCountInput) return;
    const total = Math.max(1, totalChapters || chapterList.length || (currentChapterIndex + 1) || 1);

    let start = parseInt(wfStartInput.value || '1', 10);
    let end = parseInt(wfEndInput.value || String(total), 10);
    ...
    wfStartInput.value = String(start);
    wfEndInput.value = String(end);

    let minCount = parseInt(wfMinCountInput.value || '2', 10);
    if (Number.isNaN(minCount)) minCount = 2;
    wfMinCountInput.value = clamp(minCount, 1, 20);
}

function renderWordFrequency(items) {
    if (!wfResultsEl) return;
    if (!items.length) {
        wfResultsEl.innerHTML = '<div class="chapter-list-empty show">当前范围内暂无可用词频结果。</div>';
        return;
    }

    const rows = items.map((item, idx) => `
        <div class="wf-item">
            <div class="wf-rank">#${idx + 1}</div>
            <div class="wf-word">[${escapeHTML(item.lang || '')}] ${escapeHTML(item.word)}</div>
            <div class="wf-meta">${item.count} 次 · ${item.coverage} 章</div>
        </div>
    `).join('');
    wfResultsEl.innerHTML = `<div class="wf-list">${rows}</div>`;
}

function setWordFrequencyRangeRecent(chapterCount = 5) {
    if (!wfStartInput || !wfEndInput) return;
    const total = Math.max(1, totalChapters || chapterList.length || (currentChapterIndex + 1) || 1);
    const end = clamp(currentChapterIndex + 1, 1, total);
    const start = clamp(end - chapterCount + 1, 1, total);
    wfStartInput.value = String(start);
    wfEndInput.value = String(end);
}
...
async function runWordFrequency() {
    if (wfRunning) return;
    if (!wfResultsEl) return;
    normalizeWordFrequencyInputs();

    const useJa = !!(wfLangJaInput && wfLangJaInput.checked);
    const useEn = !!(wfLangEnInput && wfLangEnInput.checked);
    if (!useJa && !useEn) {
        setWfStatus('请至少选择一种语言。');
        return;
    }

    const total = Math.max(1, totalChapters || chapterList.length || (currentChapterIndex + 1) || 1);
    const startChapter = parseInt((wfStartInput && wfStartInput.value) || '1', 10);
    const endChapter = parseInt((wfEndInput && wfEndInput.value) || String(total), 10);
    const minCount = parseInt((wfMinCountInput && wfMinCountInput.value) || '2', 10);
    const start = clamp(startChapter, 1, total);
    const end = clamp(endChapter, 1, total);

    wfRunning = true;
    if (btnWfRun) btnWfRun.disabled = true;
    setWfStatus(`正在统计第 ${Math.min(start, end)}-${Math.max(start, end)} 章...`);
    wfResultsEl.innerHTML = '<div class="chapter-list-empty show">正在分析词频，请稍候…</div>';

    try {
        const params = new URLSearchParams();
        params.append('book', BOOK_ID);
        params.append('start', String(Math.min(start, end)));
        params.append('end', String(Math.max(start, end)));
        params.append('minCount', String(clamp(minCount, 1, 20)));
        params.append('limit', '400');
        const langs = [];
        if (useJa) langs.push('ja');
        if (useEn) langs.push('en');
        params.append('lang', langs.join(','));

        const resp = await fetch(`word_freq.php?${params.toString()}&_t=${Date.now()}`);
        const data = await resp.json();
        if (!resp.ok || !data || data.ok !== true) {
            throw new Error((data && data.error) ? data.error : 'word_freq_failed');
        }
        const list = Array.isArray(data.items) ? data.items : [];

        renderWordFrequency(list);
        if (data.range && typeof data.range.start === 'number' && typeof data.range.end === 'number') {
            const elapsed = data.meta && typeof data.meta.elapsedMs === 'number' ? `，耗时 ${data.meta.elapsedMs}ms` : '';
            setWfStatus(`已完成：第 ${data.range.start}-${data.range.end} 章，共 ${list.length} 个词${elapsed}。`);
        } else {
            setWfStatus(`已完成，展示 ${list.length} 个词。`);
        }
    } catch (e) {
        console.warn('runWordFrequency error', e);
        wfResultsEl.innerHTML = '<div class="chapter-list-empty show">词频统计失败，请稍后重试。</div>';
        setWfStatus(`统计失败：${e && e.message ? e.message : 'unknown_error'}`);
    } finally {
        wfRunning = false;
        if (btnWfRun) btnWfRun.disabled = false;
    }
}

function normalizeForTitleMatch(str) {
    return String(str || '')
        .trim()
        .replace(/[\s\u3000]+/g, '')
        .replace(/[，。！？：；、“”‘’（）()\[\]【】《》＜＞<>「」『』·—\-]/g, '')
...
const TITLE_IGNORED_REGEX = /[\s\u3000，。！？：；、“”‘’（）()\[\]【】《》＜＞<>「」『』·—\-]/;

function removeTitlePrefixFromLine(line, normTitle) {
    if (!normTitle) return null;

    let normIndex = 0;
    let i = 0;

    for (; i < line.length && normIndex < normTitle.length; i++) {
        const ch = line[i];
        if (TITLE_IGNORED_REGEX.test(ch)) {
            continue;
        }

        const lower = ch.toLowerCase();
        if (normTitle[normIndex] !== lower) {
            return null;
        }
...
function stripDuplicatedTitle(rawText, title) {
    ...
    nonEmpty.forEach(idx => {
        const normalizedLine = normalizeForTitleMatch(lines[idx]);
        if (normalizedLine === normTitle) {
            toRemove.push(idx);
            return;
        }

        const remainder = removeTitlePrefixFromLine(lines[idx], normTitle);
        if (remainder !== null) {
            if (remainder.trim() === '') {
                toRemove.push(idx);
            } else {
...
function normalizeCacheInputs() {
    if (!cacheStartInput || !cacheEndInput) return;
    const total = totalChapters || chapterList.length;
    const available = Math.max(total, 1);
    let start = parseInt(cacheStartInput.value, 10);
    if (Number.isNaN(start)) start = 1;
    start = clamp(start, 1, available);
    cacheStartInput.value = start;
    let end = parseInt(cacheEndInput.value, 10);
    if (Number.isNaN(end)) end = start;
    end = clamp(end, start, available);
    cacheEndInput.value = end;
}

function syncCacheInputs({ forceDefaults = false } = {}) {
    if (!cacheStartInput || !cacheEndInput) return;
    const total = totalChapters || chapterList.length;
    const available = Math.max(total, 1);
    cacheStartInput.min = 1;
    cacheEndInput.min = 1;
    cacheStartInput.max = available;
    cacheEndInput.max = available;

    if (forceDefaults || !cacheStartInput.value) {
        cacheStartInput.value = clamp(currentChapterIndex + 1, 1, available);
    }
    if (forceDefaults || !cacheEndInput.value) {
        const defaultEnd = Math.min(available, (parseInt(cacheStartInput.value, 10) || 1) + 19);
        cacheEndInput.value = Math.max(defaultEnd, parseInt(cacheStartInput.value, 10));
    }

    normalizeCacheInputs();
    const disabled = !total;
    cacheStartInput.disabled = cacheEndInput.disabled = disabled;
    updateCacheButtonState();
}

function updateCacheButtonState() {
    if (!btnCacheRange) return;
    const total = totalChapters || chapterList.length;
    btnCacheRange.disabled = isCachingRange || !total;
}
...
const READABLE_CHAR_REGEX = /[A-Za-z0-9\u3040-\u30FF\u3400-\u9FFF]/g;
const LOG_ENDPOINT = 'reading_log.php?action=append';
const readerBookTitle = (typeof BOOK_TITLE === 'string' && BOOK_TITLE.trim()) ? BOOK_TITLE.trim() : BOOK_ID;

let readingSession = null;
let readingFlushTimer = null;
...
function countReadableChars(html) {
    ...
    const hit = text.match(READABLE_CHAR_REGEX);
    return hit ? hit.length : 0;
}

function createReadingSession(ts) {
    return {
        id: buildSessionId(),
        bookId: BOOK_ID,
        bookTitle: readerBookTitle,
        startAt: ts,
        endAt: ts,
        activeSec: 0,
        pagesRead: 0,
        charsRead: 0,
        chapterFrom: currentChapterIndex,
        pageFrom: currentPageIndex,
        chapterTo: currentChapterIndex,
...
function buildSessionPayload(session) {
    ...
    data.append('book', session.bookId);
    data.append('startAt', session.startAt);
    data.append('endAt', session.endAt);
    data.append('activeSec', session.activeSec);
    data.append('pagesRead', session.pagesRead);
    data.append('charsRead', session.charsRead);
    data.append('chapterFrom', session.chapterFrom);
    data.append('pageFrom', session.pageFrom);
    data.append('chapterTo', session.chapterTo);
    data.append('pageTo', session.pageTo);
    return data;
}

function postSession(session, useBeacon = false) {
    if (!session || session.activeSec <= 0 || session.charsRead <= 0) {
        return;
    }
    ...
    fetch(LOG_ENDPOINT, {
        ...
        console.warn('postSession error', err);
    });
}

function scheduleReadingFlush() {
    if (readingFlushTimer) return;
    readingFlushTimer = setTimeout(() => {
        readingFlushTimer = null;
        flushReadingSession({ reset: false, useBeacon: false });
    }, READING_LOG_CONFIG.flushIntervalMs);
}

function flushReadingSession({ reset = false, useBeacon = false } = {}) {
    if (!readingSession) return;

    if (readingDirty) {
        postSession(readingSession, useBeacon);
        readingDirty = false;
    }

    if (readingFlushTimer) {
        clearTimeout(readingFlushTimer);
        readingFlushTimer = null;
    }
    ...
    if (reset) {
        ...
        scheduleReadingFlush();
    }
}

function ensureReadingSession(now) {
    if (!readingSession) {
        readingSession = createReadingSession(now);
        return;
    }

    if ((now - readingSession.endAt) > READING_LOG_CONFIG.sessionGapSec) {
        flushReadingSession({ reset: true, useBeacon: false });
        readingSession = createReadingSession(now);
    }
}

function trackCurrentPageRead() {
    if (isStudyMode) return;

    const html = currentPages[currentPageIndex];
    if (!html) return;

    const nowMs = Date.now();
    const pageKey = `${currentChapterIndex}:${currentPageIndex}`;
    if (pageKey === lastReadMarkKey && (nowMs - lastReadMarkAt) < READING_LOG_CONFIG.dedupeMs) {
        return;
    }

    const now = Math.floor(nowMs / 1000);
    ensureReadingSession(now);

    if (!readingSession) return;

    const gap = Math.max(0, now - readingSession.endAt);
    if (gap < READING_LOG_CONFIG.minStepSec) {
        // 过快翻页按“浏览/定位”处理，不计入阅读量，避免速度失真
        readingSession.endAt = now;
        return;
    }

    readingSession.activeSec += Math.min(gap, READING_LOG_CONFIG.maxStepSec);
    readingSession.endAt = now;
    readingSession.pagesRead += 1;
    readingSession.charsRead += countReadableChars(html);
    readingSession.chapterTo = currentChapterIndex;
    readingSession.pageTo = currentPageIndex;

    readingDirty = true;
    lastReadMarkKey = pageKey;
    lastReadMarkAt = nowMs;
...
function processChapterPayload(index, data) {
    if (!data || typeof index !== 'number' || index < 0) return null;
    const remoteTotal = (typeof data.total === 'number' && data.total > 0) ? data.total : null;
    if (remoteTotal !== null) {
        totalChapters = remoteTotal;
    } else if (!totalChapters && chapterList.length) {
        totalChapters = chapterList.length;
    }

    const chapterTitle = data.title || ('第 ' + (index + 1) + ' 章');
    const rawText = data.content || '';
    const cleanText = stripDuplicatedTitle(rawText, chapterTitle);
    const pages = paginateChapter(cleanText);
    chapterCache[index] = {
        title: chapterTitle,
        text: cleanText,
        pages: pages,
...
function loadChapterFromServer(chapterIndex, pageToOpen = 0, direction = 0) {
    ...
    fetch(`chapter.php?book=${encodeURIComponent(BOOK_ID)}&index=${chapterIndex}&_t=${Date.now()}`)
        ...
        .then(data => {
            ...
            const serverIndex = (typeof data.index === 'number') ? data.index : chapterIndex;
            const stored = processChapterPayload(serverIndex, data);
            if (!stored) {
                throw new Error('chapter_parse_failed');
            }

            currentChapterIndex = serverIndex;
            currentPages = stored.pages || [];
            if (pageToOpen >= currentPages.length) pageToOpen = currentPages.length - 1;
            currentPageIndex = Math.max(0, pageToOpen);
            syncCacheInputs();
            updateCacheButtonState();
...
function loadChapterList() {
    ...
    return fetch(`chapter_list.php?book=${encodeURIComponent(BOOK_ID)}&_t=${Date.now()}`)
        ...
        .then(data => {
            ...
            renderChapterRanges();
            syncCacheInputs({ forceDefaults: true });
            setCacheStatus('');
            updateCacheButtonState();
        })
        .catch(err => {
...
async function cacheChapterRange() {
    if (!btnCacheRange || isCachingRange) return;
    if (!cacheStartInput || !cacheEndInput) return;
    ...
    const totalToCache = endIndex - startIndex + 1;
    if (totalToCache <= 0) {
        setCacheStatus('范围无效');
        return;
    }
    ...
    isCachingRange = true;
    updateCacheButtonState();
    setCacheStatus(`正在缓存 0/${totalToCache} 章`);

    let cachedCount = 0;
    for (let idx = startIndex; idx <= endIndex; idx++) {
        if (chapterCache[idx] && Array.isArray(chapterCache[idx].pages) && chapterCache[idx].pages.length) {
            cachedCount++;
            setCacheStatus(`跳过已缓存 ${cachedCount}/${totalToCache} 章`);
            continue;
        }

        try {
            const response = await fetch(`chapter.php?book=${encodeURIComponent(BOOK_ID)}&index=${idx}&_t=${Date.now()}`);
            if (!response.ok) {
            ...
            const targetIndex = (typeof data.index === 'number') ? data.index : idx;
            const stored = processChapterPayload(targetIndex, data);
            if (!stored) {
                throw new Error('章节解析失败');
            }
            cachedCount++;
            setCacheStatus(`缓存中 ${cachedCount}/${totalToCache} 章`);
        } catch (error) {
            console.error(error);
            setCacheStatus(`第 ${idx + 1} 章失败`);
            showToast(`缓存失败：${error.message || '网络错误'}`);
            break;
        }
    }

    if (cachedCount === totalToCache) {
        setCacheStatus(`缓存完成 ${cachedCount}/${totalToCache} 章`);
        showToast('缓存完成');
    }

    isCachingRange = false;
    updateCacheButtonState();
}
...
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'hidden') {
        flushReadingSession({ reset: true, useBeacon: true });
    }
});
...
if (cacheStartInput) {
    cacheStartInput.addEventListener('input', normalizeCacheInputs);
    cacheStartInput.addEventListener('blur', normalizeCacheInputs);
}

if (cacheEndInput) {
    cacheEndInput.addEventListener('input', normalizeCacheInputs);
    cacheEndInput.addEventListener('blur', normalizeCacheInputs);
}

if (btnCacheRange) {
    btnCacheRange.addEventListener('click', (e) => {
        e.preventDefault();
        cacheChapterRange();
    });
}

if (settingsTabReadingEl) {
    settingsTabReadingEl.addEventListener('click', (e) => {
        e.preventDefault();
        switchSettingsTab('reading');
    });
}

if (settingsTabVocabEl) {
    settingsTabVocabEl.addEventListener('click', (e) => {
        e.preventDefault();
...
if (wfStartInput) {
    wfStartInput.addEventListener('input', normalizeWordFrequencyInputs);
    wfStartInput.addEventListener('blur', normalizeWordFrequencyInputs);
}

if (wfEndInput) {
    wfEndInput.addEventListener('input', normalizeWordFrequencyInputs);
    wfEndInput.addEventListener('blur', normalizeWordFrequencyInputs);
}

if (wfMinCountInput) {
    wfMinCountInput.addEventListener('input', normalizeWordFrequencyInputs);
    wfMinCountInput.addEventListener('blur', normalizeWordFrequencyInputs);
}

if (btnWfRecent5) {
    btnWfRecent5.addEventListener('click', (e) => {
        e.preventDefault();
        setWordFrequencyRangeRecent(5);
        normalizeWordFrequencyInputs();
    });
...
if (btnWfRun) {
    btnWfRun.addEventListener('click', (e) => {
        e.preventDefault();
        runWordFrequency();
    });
}
